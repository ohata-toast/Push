...
### 2017.04.20
#### 기능 개선/변경
* [CONSOLE] 메시지 발송 차트 라이브러리 변경
    * 기존 메시지 발송 차트 라이브러리를 high chart에서 tui.chart로 변경했습니다.

#### 기능 추가
* [API] 메시지 수신, 확인 데이터 수집(Message Delivery Receipt), 통계 조회 기능 추가
    * 메시지 발송 후, 기기에 수신, 사용자의 메시지 확인 데이터를 수집해 통계로 조회할 수 있는 기능을 추가했습니다.
...
* [API] v2.0 API 추가
    * 토큰 통계 API, 예약 메시지 API가 추가했습니다.
    * 메시지 수신, 확인 통계 조회 API가 추가했습니다.
    * 피드백 API는 유효하지 않은 토큰 API로 변경했습니다.  
