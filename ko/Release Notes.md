...
* [SDK] 토큰 변경이 없어도 oldToken을 설정해 토큰을 등록하는 버그 수정
   * 위 버그 수정과 관련된 버그 수정입니다.
	* 토큰 변경이 없어도 oldToken에 값을 설정해 token에 설정된 값과 같은 값으로 등록 시도하는 버그를 수정했습니다.
	* 토큰 변경이 있을때만 oldToken에 값을 설정합니다.
